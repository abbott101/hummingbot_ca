...
from hummingbot.connector.exchange.ccex import ccex_constants as CONSTANTS, ccex_utils, ccex_web_utils as web_utils
from hummingbot.connector.exchange.ccex.ccex_api_order_book_data_source import CcexAPIOrderBookDataSource
from hummingbot.connector.exchange.ccex.ccex_api_user_stream_data_source import CcexAPIUserStreamDataSource
...
class CcexExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self.api_key = ccex_api_key
        self.secret_key = ccex_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_ccex_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def ccex_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(ccex_type: str) -> OrderType:
        return OrderType[ccex_type]
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return CcexAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return CcexAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        type_str = CcexExchange.ccex_order_type(order_type)
       ...
       #     api_params["timeInForce"] = CONSTANTS.TIME_IN_FORCE_GTC
        ...
        #     resp = await client.post(CONSTANTS.API_AGENT_BASE+CONSTANTS.API_AGENT_NEWORDER)
        #     account_info = await resp.json()

        order_result = await self._api_post(
            path_url=CONSTANTS.API_AGENT_NEWORDER,
            data=api_params,
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(ccex_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/ccex-exchange/ccex-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Ccex the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Ccex's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Ccex's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_ccex_timestamp * 1e3)
            self._last_trades_poll_ccex_timestamp = self._time_synchronizer.time()
    ...
    async def _update_balances(self):
        ...
        async with aiohttp.ClientSession() as client:
            resp = await client.get(CONSTANTS.API_AGENT_BASE+CONSTANTS.API_AGENT_BALANCE)
            account_info = await resp.json()

        # self._account_available_balances["USDT"] = Decimal(5.4)
        # self._account_balances["USDT"] = Decimal(100)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(ccex_utils.is_exchange_information_valid, exchange_info["symbols"]):
